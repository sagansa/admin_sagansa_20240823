<!DOCTYPE html>
<html>
<head>
    <title>Pesanan Langsung Baru</title>
</head>
<body style="font-family: Arial, sans-serif; line-height: 1.6; color: #333;">
    <div style="max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #ddd; border-radius: 8px;">
        <h2 style="color: #4CAF50; border-bottom: 2px solid #eee; padding-bottom: 10px;">
            Notifikasi Transaksi Baru
        </h2>
        
        <p>Halo,</p>
        
        <p>Pesanan Sales Order Direct baru saja berhasil dibuat. Berikut adalah ringkasan transaksinya:</p>
        
        <table style="width: 100%; border-collapse: collapse; margin-top: 15px;">
            <tr>
                <td style="padding: 8px; border-bottom: 1px solid #eee; width: 40%;"><strong>Order ID:</strong></td>
                <td style="padding: 8px; border-bottom: 1px solid #eee;">#{{ $order->id }}</td>
            </tr>
            <tr>
                <td style="padding: 8px; border-bottom: 1px solid #eee;"><strong>Tanggal Pesanan:</strong></td>
                <td style="padding: 8px; border-bottom: 1px solid #eee;">{{ \Carbon\Carbon::parse($order->created_at)->format('d F Y, H:i') }}</td>
            </tr>
            <tr>
                <td style="padding: 8px; border-bottom: 1px solid #eee;"><strong>Pemesan (Customer):</strong></td>
                <td style="padding: 8px; border-bottom: 1px solid #eee;">{{ $order->orderedBy?->name ?? 'Unknown' }}</td>
            </tr>
            <tr>
                <td style="padding: 8px; border-bottom: 1px solid #eee;"><strong>Total Jumlah:</strong></td>
                <td style="padding: 8px; border-bottom: 1px solid #eee;">Rp {{ number_format($order->total_price, 0, ',', '.') }}</td>
            </tr>
        </table>
        
        <p style="margin-top: 25px;">
            Silakan periksa halaman dashboard admin Anda untuk melihat detail pesanan dan mengatur pengiriman.
        </p>

        <p style="font-size: 0.9em; color: #777; margin-top: 30px; border-top: 1px solid #eee; padding-top: 10px;">
            Email ini dibuat secara otomatis oleh sistem. Harap tidak membalas email ini.
        </p>
    </div>
</body>
</html>
